...
if sys.version_info[0] == 2:
    ...
    from tkinter import ( Frame, Label, LabelFrame, Entry, OptionMenu,
                          ...
                          PhotoImage, Canvas, Scrollbar, Wm, TclError,
                          StringVar, IntVar, E, W, EW, NW, Y, VERTICAL, SOLID,
                          CENTER, RIGHT, LEFT, BOTH, TRUE, FALSE, END )
...
class FoTDeviceDialog(tkSimpleDialog.Dialog):
    #Definir parametros e desenhar tela
    #SensorName
    #Dataset Path
    #Gateway name
    #Gateway IP
    def __init__(self, parent, title, prefDefaults=None,allNodes=None):
    ...
    def body(self, master):
        ...
        self.dataOption = StringVar(value="random")
        self.var = StringVar(master)
        ...
        #opcoes
        Label(master, text="Data Source:").grid(row=rowCount, sticky=E)
        self.radioRandom = Radiobutton(
            master, text="Usar dados randômicos",
            variable=self.dataOption, value="random",
            command=self.toggleDataset
        )
        self.radioRandom.grid(row=rowCount, column=1, sticky=W)
        rowCount += 1

        self.radioDataset = Radiobutton(
            master, text="Usar dataset",
            variable=self.dataOption, value="dataset",
            command=self.toggleDataset
        )
        self.radioDataset.grid(row=rowCount, column=1, sticky=W)
        rowCount += 1
        
        self.datasetLabel = Label(master, text="Dataset Path:")
        self.datasetEntry = Entry(master)
        self.datasetButton = Button(master, text="Browse", command=self.selectFile)
        # NÃO chamar grid ainda (fica oculto)
        
        self.datasetRow = rowCount
        rowCount += 1
        
        #criando opcoes em caso de csv
        self.columnLabel = Label(master, text="Escolha qual dado será usado:")
        self.columnVar = StringVar()

        self.columnMenu = ttk.Combobox(
            master,
            textvariable=self.columnVar,
            state="readonly"  
        )

        self.columnRow = rowCount
        rowCount += 1
        
        
        # Field for SensorName
        Label(master, text="Sensor Name:").grid(row=rowCount, sticky=E)
        
        
        
        self.sensorNameVar = StringVar()
        self.sensorName = ttk.Combobox(
            master,
            textvariable=self.sensorNameVar,
            state="readonly"  # impede digitação manual
        )
        self.sensorName['values'] = (
            "humiditySensor",
            "temperatureSensor",
            "soilmoistureSensor",
        ...
        self.sensorName.grid(row=rowCount, column=1)
        self.sensorName.current(1)
        rowCount+=1
        
        Label(master, text="FoT-Gateway Node Name:").grid(row=rowCount, sticky=E)
        
        self.gatewayNameVar = StringVar()
        self.gatewayNameNode = ttk.Combobox(
            master,
            textvariable=self.gatewayNameVar,
            state="readonly"  # impede digitação manual
        )
        
        self.configureGatewayName()
        
        self.gatewayNameNode.current(0)
        self.gatewayNameNode.grid(row=rowCount, column=1)
        rowCount+=1
        #Todo: add option to external communication 
        
        Label(master, text="Publish Time(seconds):").grid(row=rowCount, sticky=E)
        self.publishTime = Entry(master)
        self.publishTime.grid(row=rowCount, column=1)
        rowCount+=1
        
        
        
        #alerta
        self.FoTDeviceAllert = Label(master, text='', fg='red', anchor='center', justify='center')
        self.FoTDeviceAllert.grid(row=rowCount, column=0, columnspan=2, pady=5)
        self.FoTDeviceAllert.config(text="")
        rowCount+=1
    
    
    def configureGatewayName(self):
        selectedNodes=[]
        if(self.allNodes):
            for key,item in self.allNodes.items():
                if(item['fotNode']):
                    if(item['fotNode']=="FoTGateway"):
                        selectedNodes.append(item['hostname'])
            
            for key,item in self.allNodes.items():
                if(item['fotNode']):
                    if(item['fotNode']=="ExternalGateway"):
                        selectedNodes.append(item['hostname']+" (External Gateway)")
        
        self.gatewayNameNode['values'] = selectedNodes
        if(len(selectedNodes)==0):
            self.gatewayNameNode['values']=("Empty")
        
        
    def selectFile(self):
        import csv

        file_path = tkFileDialog.askopenfilename(filetypes=[("CSV files", "*.csv")])

        if file_path:
            self.datasetEntry.delete(0, END)
            self.datasetEntry.insert(0, file_path)
            ...
            try:
                ...
                self.updateColumnOptions(headers)

            except Exception as e:
                self.FoTDeviceAllert.config(
                    text="Erro ao ler CSV!", fg="red"
                )
    
    def updateColumnOptions(self, columns):
        # Define valores diretamente
        self.columnMenu['values'] = columns
        ...
        # Define valor padrão
        if columns:
            self.columnMenu.current(0)

        # Mostrar widgets
        self.columnLabel.grid(row=self.columnRow, column=0, sticky=E)
        self.columnMenu.grid(row=self.columnRow, column=1, sticky="we")
    
    def toggleDataset(self):
        if self.dataOption.get() == "dataset":
            self.datasetLabel.grid(row=self.datasetRow, column=0, sticky=E)
            self.datasetEntry.grid(row=self.datasetRow, column=1)
            self.datasetButton.grid(row=self.datasetRow, column=2)
        else:
            self.datasetLabel.grid_remove()
            self.datasetEntry.grid_remove()
            self.datasetButton.grid_remove()

            # esconder seleção de coluna também
            self.columnLabel.grid_remove()
            self.columnMenu.grid_remove()
    
    def apply(self):        
        self.result = {}
        self.result['sensorName'] = self.sensorNameVar.get()
        self.result['gatewayNameNode'] = str(self.gatewayNameNode.get())
        self.result['gatewayNameNode'] = self.result['gatewayNameNode'].replace(" (External Gateway)", "").strip()
        self.result['publishTime'] = str(self.publishTime.get())
        
        self.result['dataOption'] = self.dataOption.get()
        
        
        if self.dataOption.get() == "dataset":
            self.result['datasetPath'] = self.datasetEntry.get()
            self.result['datasetColumn'] = self.columnVar.get()
            
        print(self.result)
        
    def validate(self):
        if not self.sensorNameVar.get():
            self.FoTDeviceAllert.config(text="Selecione um Sensor!", fg="red")
            return False
        
        if self.gatewayNameVar.get()=="Empty":
            self.FoTDeviceAllert.config(text="Nenhum Gateway selecionado.\nPrimeiro você deve criar um FoTGateway!", fg="red")
            return False

        if not self.gatewayNameNode.get().strip():
            self.FoTDeviceAllert.config(text="Gateway Name é obrigatório!", fg="red")
            return False

        publish_value = self.publishTime.get().strip()

        if not publish_value:
            self.FoTDeviceAllert.config(text="Publish Time é obrigatório!", fg="red")
            return False

        try:
            publish_int = int(publish_value)
            if publish_int < 2:
                self.FoTDeviceAllert.config(
                    text="Publish Time deve ser maior que 1!", fg="red"
                )
                return False
        except ValueError:
            self.FoTDeviceAllert.config(
                text="Publish Time deve ser um número inteiro!", fg="red"
            )
            return False
        
        if self.dataOption.get() == "dataset":
            if not self.datasetEntry.get().strip():
                self.FoTDeviceAllert.config(text="Selecione um dataset!", fg="red")
                return False

            if not self.columnVar.get():
                self.FoTDeviceAllert.config(text="Escolha uma coluna do dataset!", fg="red")
                return False
...
class ExternalGatewayDialog(tkSimpleDialog.Dialog):
    def __init__(self, parent, title, prefDefaults=None,allNodes=None,nodeName=None):
    ...
    def body(self, master):
        ...
        self.var = StringVar(master)
        self.protcolvar = StringVar(master)
        self.placeholderMQTT="1883"
        rowCount=0
        
        Label(master, text="External Gateway IP:").grid(row=rowCount, sticky=E)
        self.externalGatewayIp = Entry(master)
        self.externalGatewayIp.grid(row=rowCount, column=1)
        rowCount+=1
        
        Label(master, text="MQTT Server Port:").grid(row=rowCount, sticky=E)
        self.externalMQTTPort = Entry(master)
        self.externalMQTTPort.grid(row=rowCount, column=1)
        rowCount+=1
        self.addPlaceholderMQTT()
        
        self.externalMQTTPort.bind("<FocusIn>", self.focusIn)
        self.externalMQTTPort.bind("<FocusOut>", self.focusOut)
    ...
    def focusIn(self,event):
        if self.externalMQTTPort.get() == self.placeholderMQTT:
            self.externalMQTTPort.delete(0, 'end')
            self.externalMQTTPort.config(fg='black')
        
        
    def focusOut(self,event):
        if not self.externalMQTTPort.get():
            self.externalMQTTPort.insert(0, self.placeholderMQTT)
            self.externalMQTTPort.config(fg='grey')
    
    def addPlaceholderMQTT(self):
        self.externalMQTTPort.insert(0, self.placeholderMQTT)
        self.externalMQTTPort.config(fg='grey')
        
    def apply(self):        
        self.result = {}
        self.result['externalGatewayIp'] = str(self.externalGatewayIp.get())
        if(self.externalMQTTPort.get()==""):
            self.result['externalMQTTPort']=self.placeholderMQTT
        else:
            self.result['externalMQTTPort'] = str(self.externalMQTTPort.get())
        self.result['hostName'] = self.nodeName
            
    
        
    def validate(self):
        
        if not self.externalGatewayIp.get():
            self.FoTDeviceAllert.config(text="Digite um endereço de IP", fg="red")
            return False
...
class MiniEdit( Frame ):
    ...
    def __init__( self, parent=None, cheight=600, cwidth=1000 ):
        ...
        self.buttons = {}
        self.active = None
        self.tools = ( 'Select', 'Host', 'Switch', 'LegacySwitch', 'FoTDevice','FoTGateway','ExternalGateway', 'NetLink', 'Controller' )
        ...
        self.nodePrefixes = { 'FoTDevice': 'd', 'LegacySwitch': 's', 'Switch': 's', 'Host': 'h' , 'Controller': 'c', 'FoTGateway': 'g','ExternalGateway': 'eg'}
        self.widgetToItem = {}
        self.itemToWidget = {}
        ...
        self.FoTDeviceRunPopup = Menu(self.top, tearoff=0)
        self.FoTDeviceRunPopup.add_command(label='FoTDevice Options', font=self.font)
        self.FoTDeviceRunPopup.add_separator()
        self.FoTDeviceRunPopup.add_command(label='Terminal', font=self.font, command=self.xterm )
        
        #command precisa mudar para self.FoTDeviceDetails
        self.FoTDevicePopup = Menu(self.top, tearoff=0)
        self.FoTDevicePopup.add_command(label='FoTDevice Options', font=self.font)
        self.FoTDevicePopup.add_separator()
        self.FoTDevicePopup.add_command(label='Properties', font=self.font, command=self.FoTDeviceDetails )
        
        self.ExternalGatewayPopup = Menu(self.top, tearoff=0)
        self.ExternalGatewayPopup.add_command(label='ExternalGateway Options', font=self.font)
        self.ExternalGatewayPopup.add_separator()
        self.ExternalGatewayPopup.add_command(label='Properties', font=self.font, command=self.ExternalGatewayDetails )
        
        self.FoTGatewayPopup = Menu(self.top, tearoff=0)
        self.FoTGatewayPopup.add_command(label='FoTGateway Options', font=self.font)
        self.FoTGatewayPopup.add_separator()
        self.FoTGatewayPopup.add_command(label='Properties', font=self.font, command=self.xterm )
        
        self.FoTGatewayRunPopup = Menu(self.top, tearoff=0)
        self.FoTGatewayRunPopup.add_command(label='FoTGateway Options', font=self.font)
        self.FoTGatewayRunPopup.add_separator()
        self.FoTGatewayRunPopup.add_command(label='Terminal', font=self.font, command=self.xterm )
        self.FoTGatewayRunPopup.add_command(label='Real Time Log', font=self.font, command=self.xtermLogMosquitto )
        ...
        self.switchOpts = {}
        self.hostCount = 0
        self.fotDeviceCount = 0
        self.externalGatewayCount = 0
        self.gatewayCount = 0
        self.switchCount = 0
    ...
    def doRun(self):
        ...
        # executa depois de 5 segundos
        self.after(5000, self.finishRun)
    ...
    def hideLoadingScreen(self):
        ...
        if hasattr(self, 'loadingOverlay'):
            ...
            del self.loadingOverlay
    
    
    def animateLoadingText(self):

        dots = "." * (self.loadingCounter % 4)

        self.loadingLabel.config(
            text="Aguarde, configurando o simulador" + dots
        )

        self.loadingCounter += 1

        # continua animando enquanto overlay existir
        if hasattr(self, 'loadingOverlay'):
            self.loadingOverlay.after(500, self.animateLoadingText)
    ...
    def showLoadingScreen(self):
        ...
        # overlay
        self.loadingOverlay = Toplevel(self)

        # remove barra
        self.loadingOverlay.overrideredirect(True)

        # bloqueia interação
        self.loadingOverlay.grab_set()

        # pega geometria da janela principal
        root_x = self.top.winfo_rootx()
        root_y = self.top.winfo_rooty()

        root_w = self.top.winfo_width()
        root_h = self.top.winfo_height()

        self.loadingOverlay.geometry(
            "%dx%d+%d+%d" % (
                root_w,
                root_h,
                root_x,
        ...
        # fundo escuro
        self.loadingOverlay.configure(bg='black')

        # transparência
        self.loadingOverlay.attributes("-alpha", 0.75)

        # label animado
        self.loadingLabel = Label(
            self.loadingOverlay,
            text="Aguarde, configurando o simulador",
    ...
    def canvasHandle( self, eventName, event ):
        ...
        #print("handler",eventName + toolName)
    ...
    def deleteItem( self, item ):
        ...
        #    name = widget[ 'text' ]
        #    print(name)
            #criar função para apagar o item
    ...
    def getTotalHosts(self):
        total = 0
        for item in self.hostOpts.values():
            if 'nodeNum' in item:
                total += item['nodeNum']
        return total
    ...
    def newNode( self, node, event ):
        ...
        if node == 'FoTDevice':
            self.fotDeviceCount += 1
            name = self.nodePrefixes[ node ] + str( self.fotDeviceCount )
            self.hostOpts[name] = {'sched':'host'}
            self.hostOpts[name]['nodeNum']=self.fotDeviceCount
            self.hostOpts[name]['hostname']=name
            self.hostOpts[name]['fotNode']=node
        if node == 'FoTGateway':
            self.gatewayCount += 1
            name = self.nodePrefixes[ node ] + str( self.gatewayCount )
            self.hostOpts[name] = {'sched':'host'}
            self.hostOpts[name]['nodeNum']=self.gatewayCount
            self.hostOpts[name]['hostname']=name
            self.hostOpts[name]['fotNode']=node
        if node == 'ExternalGateway':
            self.externalGatewayCount += 1
            name = self.nodePrefixes[ node ] + str( self.externalGatewayCount )
            self.hostOpts[name] = {'sched':'host'}
            self.hostOpts[name]['nodeNum']=self.externalGatewayCount
            self.hostOpts[name]['hostname']=name
            self.hostOpts[name]['fotNode']=node
        ...
        if node == 'Host':
            ...
            self.hostOpts[name]['hostname']=name
            self.hostOpts[name]['fotNode']=node
        if(node == 'Host'or node == 'FoTDevice' or node == 'FoTGateway'):
            self.hostOpts[name]['nodeNumIp']=self.getTotalHosts()
        if node == 'Controller':
        ...
        if node == 'Switch':
            icon.bind('<Button-3>', self.do_switchPopup )
        if node == 'FoTDevice':
            icon.bind('<Button-3>', self.do_FoTDevicePopup )
        if node == 'ExternalGateway':
            icon.bind('<Button-3>', self.do_ExternalGatewayPopup )
        if node == 'FoTGateway':
            icon.bind('<Button-3>', self.do_FoTGatewayPopup )
        if node == 'LegacySwitch':
            icon.bind('<Button-3>', self.do_legacySwitchPopup )
        if node == 'Host':
            icon.bind('<Button-3>', self.do_hostPopup )
    ...
    def clickFoTDevice( self, event ):
        "Add a new switch to our canvas."
        self.newNode( 'FoTDevice', event )

    def clickFoTGateway( self, event ):
        "Add a new switch to our canvas."
        self.newNode( 'FoTGateway', event )
        
    def clickExternalGateway( self, event ):
        "Add a new switch to our canvas."
        self.newNode( 'ExternalGateway', event )
    ...
    def finishLink( self, event ):
        ...
        if (('Host' in stags and 'Host' in dtags) or
           ('Controller' in dtags and 'FoTDevice' in stags) or
           ('Controller' in stags and 'FoTDevice' in dtags) or
           ('Controller' in dtags and 'FoTGateway' in stags) or
           ('Controller' in stags and 'FoTGateway' in dtags) or
           ('Controller' in dtags and 'ExternalGateway' in stags) or
           ('Controller' in stags and 'ExternalGateway' in dtags) or
    ...
    def about( self ):
        ...
        if about is None:
            ...
            about.title( 'About' )
            desc = self.appName + ': a simple Fog of Things editor based for MiniNet'
            version = 'Fog of Things Edit '
    ...
    def ExternalGatewayDetails ( self, _ignore=None ):
        print("ExternalGatewayDetails")
        ...
        if 'ExternalGateway' not in tags:
            return
        
        prefDefaults = self.hostOpts[name]
        allNodes=self.hostOpts
        hostBox = ExternalGatewayDialog(self, title='ExternalGateway Details', prefDefaults=prefDefaults,allNodes=allNodes,nodeName=name)
        if hostBox.result:
            self.hostOpts[name]["externalGatewayIp"]=hostBox.result['externalGatewayIp']
            self.hostOpts[name]["externalMQTTPort"]=hostBox.result['externalMQTTPort']
            print("hostOpts",self.hostOpts)
                
                
                
    def FoTDeviceDetails( self, _ignore=None ):
        if ( self.selection is None or
             self.net is not None or
             self.selection not in self.itemToWidget ):
            return
        ...
        print(name)
        tags = self.canvas.gettags( self.selection )
        if 'FoTDevice' not in tags:
            return
        print("Popup ativado")
        
        prefDefaults = self.hostOpts[name]
        allNodes=self.hostOpts
        hostBox = FoTDeviceDialog(self, title='FoTDevice Details', prefDefaults=prefDefaults,allNodes=allNodes)
        #self.master.wait_window(hostBox.top)
        if hostBox.result:
            self.hostOpts[name]['sensorName']=hostBox.result['sensorName']
            self.hostOpts[name]['gatewayNameNode']=hostBox.result['gatewayNameNode']
            self.hostOpts[name]['publishTime']=hostBox.result['publishTime']
            if('datasetPath' in hostBox.result):
                self.hostOpts[name]['datasetPath']=hostBox.result['datasetPath']
                self.hostOpts[name]['datasetColumn']=hostBox.result['datasetColumn']
            print("hostOpts",self.hostOpts)
    ...
    def deleteNode( self, item ):
        ...
        nameWidget = self.itemToWidget[ self.selection ]
        name = nameWidget[ 'text' ]
        print("Nome do device deletado",name)
        self.updateHostOpt(name)
        del self.itemToWidget[ item ]
        del self.widgetToItem[ widget ]
    
    def updateHostOpt(self,name):
        
        if name not in self.hostOpts:
            print(f"Chave '{name}' não encontrada.")
            return None
        # Remove o elemento principal
    ...
    def buildNodes( self, net):
        ...
        for widget in self.widgetToItem:
            ...
            if 'Switch' in tags:
            ...
            elif 'ExternalGateway' in tags:
                #fazer configuração de comunicação externa aqui
                pass
            ...
            elif ('Host' in tags) or ('FoTDevice' in tags) or ('FoTGateway' in tags):
                opts = self.hostOpts[name]
                # debug( str(opts), '\n' )
                ip = None
                defaultRoute = None
                ...
                if 'ip' in opts and len(opts['ip']) > 0:
                    ...
                    nodeNum = self.hostOpts[name]['nodeNumIp']
                    ipBaseNum, prefixLen = netParse( self.appPrefs['ipBase'] )
                    ip = ipAdd(i=nodeNum, prefixLen=prefixLen, ipBaseNum=ipBaseNum)
    ...
    def getNodeByType(self,nodeType):
        l=[]
        for key,item in self.hostOpts.items():
            if(item['fotNode']==nodeType):
                l.append(item)
        return l
    ...
    def buildLinks( self, net):
        ...
        for key,link in self.links.items():
            ...
            if 'data' in tags:
                ...
                linkopts=link['linkOpts']
                srcName, dstName = src[ 'text' ], dst[ 'text' ]
                if(self.getTypeByName(srcName)!="ExternalGateway" and self.getTypeByName(dstName)!="ExternalGateway"):
        ...
        for key,item in self.hostOpts.items():
                if(item['fotNode']):
                    if(item['fotNode']=="ExternalGateway"):
                        ...
                        self.rootNode = self.connectToInternet( net )
                        break    

        return net
    
    def connectToInternet(self, network, switch='s1', rootip='10.254', subnet='10.0/8'):
        ...
        self.fixNetworkManager( root, 'root-eth0' )
    ...
    def startNAT(self,root, inetIntf='eth0', subnet='10.0/8' ):
    ...
    def stopNAT(self ):
        ...
        # Flush any currently active rules
        self.rootNode.cmd( 'iptables -F' )
        self.rootNode.cmd( 'iptables -t nat -F' )

        # Instruct the kernel to stop forwarding
        self.rootNode.cmd( 'sysctl net.ipv4.ip_forward=0' )
    ...
    def init_gateways(self):
        for widget in self.widgetToItem:
            tags = self.canvas.gettags( self.widgetToItem[ widget ] )
            name = widget[ 'text' ]
            if 'FoTGateway' in tags:
                self.net.get(name).cmd("mosquitto &")
            time.sleep(1)
    ...
    def init_sensors(self):
        ...
        for widget in self.widgetToItem:
            tags = self.canvas.gettags( self.widgetToItem[ widget ] )
            name = widget[ 'text' ]
            if ('FoTDevice' in tags) and ('gatewayNameNode' in self.hostOpts[name]):
                    gatewayName=self.hostOpts[name]['gatewayNameNode']
                    if(self.hostOpts[gatewayName]['fotNode']=='ExternalGateway'):
                        if('datasetPath' in self.hostOpts[name]):
                            self.net.get(name).cmd("cd ..; python fot_devices/main_n.py --name "+name+" --broker "+str(self.hostOpts[gatewayName]['externalGatewayIp'])+" --dataset "+str(self.hostOpts[name]['datasetPath'])+" --datasetColumn "+str(self.hostOpts[name]['datasetColumn'])+" &")
                        else:
                            self.net.get(name).cmd("cd ..; python fot_devices/main_n.py --name "+name+" --broker "+str(self.hostOpts[gatewayName]['externalGatewayIp'])+" &")
                        from pathlib import Path
                        sys.path.append(str(Path.cwd().parent)+"/tatu")
                        time.sleep(0.5)
                        from TATU import TatuReq, TatuRes
                        tatuMessage = TatuReq("FLOW",collect=int(self.hostOpts[name]['publishTime'])*1000,publish=int(self.hostOpts[name]['publishTime'])*1000,sensor=self.hostOpts[name]['sensorName'],device="sc"+str(countSensors).zfill(2))
                        self.net.get(name).cmd("mosquitto_pub -h '"+str(self.hostOpts[gatewayName]['externalGatewayIp'])+"' -t 'dev/"+name+"/REQ' -m '"+tatuMessage.getTatu()+"'")
                        countSensors+=1
                    else:
                        if('datasetPath' in self.hostOpts[name]):
                            self.net.get(name).cmd("cd ..; python fot_devices/main_n.py --name "+name+" --broker "+str(self.net.get(str(self.hostOpts[name]['gatewayNameNode'])).IP())+" --dataset "+str(self.hostOpts[name]['datasetPath'])+" --datasetColumn "+str(self.hostOpts[name]['datasetColumn'])+" &")
                        else:
                            self.net.get(name).cmd("cd ..; python fot_devices/main_n.py --name "+name+" --broker "+str(self.net.get(str(self.hostOpts[name]['gatewayNameNode'])).IP())+" &")
                        from pathlib import Path
                        sys.path.append(str(Path.cwd().parent)+"/tatu")
                        time.sleep(0.5)
                        from TATU import TatuReq, TatuRes
                        tatuMessage = TatuReq("FLOW",collect=int(self.hostOpts[name]['publishTime'])*1000,publish=int(self.hostOpts[name]['publishTime'])*1000,sensor=self.hostOpts[name]['sensorName'],device="sc"+str(countSensors).zfill(2))
                        self.net.get(name).cmd("mosquitto_pub -h '"+str(self.net.get(str(self.hostOpts[name]['gatewayNameNode'])).IP())+"' -t 'dev/"+name+"/REQ' -m '"+tatuMessage.getTatu()+"'")
                        countSensors+=1
    ...
    def do_ExternalGatewayPopup(self, event):
        # display the popup menu
        if self.net is None:
            try:
                self.ExternalGatewayPopup.tk_popup(event.x_root, event.y_root, 0)
            finally:
                # make sure to release the grab (Tk 8.0a1 only)
                self.ExternalGatewayPopup.grab_release()
    
    
    def do_FoTDevicePopup(self, event):
        # display the popup menu
        if self.net is not None:
            try:
                self.FoTDeviceRunPopup.tk_popup(event.x_root, event.y_root, 0)
            finally:
                # make sure to release the grab (Tk 8.0a1 only)
                self.FoTDeviceRunPopup.grab_release()
        else:
            #Todo: adicionar aqui o FoTDevicePopup
            try:
                self.FoTDevicePopup.tk_popup(event.x_root, event.y_root, 0)
            finally:
                # make sure to release the grab (Tk 8.0a1 only)
                self.FoTDevicePopup.grab_release()
                
    def do_FoTGatewayPopup(self, event):
        # display the popup menu
        if self.net is not None:
            try:
                self.FoTGatewayRunPopup.tk_popup(event.x_root, event.y_root, 0)
            finally:
                # make sure to release the grab (Tk 8.0a1 only)
                self.FoTGatewayRunPopup.grab_release()
        else:
            #Todo: adicionar aqui o FoTGatewayPopup
            try:
                self.FoTGatewayPopup.tk_popup(event.x_root, event.y_root, 0)
            finally:
                # make sure to release the grab (Tk 8.0a1 only)
                self.FoTGatewayPopup.grab_release()
    ...
    def xtermLogMosquitto( self, _ignore=None ):
        "Make an xterm when a button is pressed."
        if ( self.selection is None or
             self.net is None or
             self.selection not in self.itemToWidget ):
        ...
        if name not in self.net.nameToNode:
            return
        term = makeTerm( self.net.nameToNode[ name ], "Real time IoT messages in", term=self.appPrefs['terminalType'],cmd=cmd)
        if StrictVersion(MININET_VERSION) > StrictVersion('2.0'):
            self.net.terms += term
        else:
            self.net.terms.append(term)
...
4BE2eIRYeHAEIBwBP0Y4Qn41YWRSCQgAOw==
        ...
        'FoTDevice': PhotoImage( data=r"""iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCA
            
            """),

        'FoTGateway': PhotoImage( data=r"""iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAQAAADZc7
        
        """),
        ...
        'ExternalGateway': PhotoImage( data=r"""
            iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAACXBIW
            XMAAADsAAAA7AF5KHG9AAAAGXRFWHRTb2Z0d2FyZQB3d3cuaW5rc2
            NhcGUub3Jnm+48GgAAAzNJREFUWIW911uoVGUUB/DfGUdFM6vT1dS
            QopRuVIhgYBZYgfhgD9FDRRlhZkIXsqewi93owpGIeigiBS0iCnuQ
            8pTRFU70EEGlqZRoghJZp9PFNKeH9U1nt5k9s/fo8Q8bZn/f2mv9v
            /Wt29A9ZmIdduJ3/Ij1uBY9h6G3I8ZiLRr4AHclo7fjdfyNT3DaSB
            gfjY3Yi8sLZKbjK2zBCYdj7CZ8jV+wR5z2SQxhRodvT8R2rO7G8Gh
            xggYG8QW+wcG0tr6kngX4B2dWJfBZMrQyt96Lj7AfV5TQ04PduLOK
            8VnJ+AttZNYKr5TBBjxfhcBz+EtcQxEuxIc4qYOuM/A5vkef4qD9H
            /qwqYxgB7wi7v8QDgivNvAzbswK1nIf/oqTcR7mYGqXBN7EfIwR3h
            yLZYnUGgXXcgw+zbBt4MUuCRShR6RzA0uyG73YJ1z2MS5Na8cfYQJ
            NEj+IEv5fyf5WuOfKETDYCucKL8yCuenlsaNkvIltWF7HUuH6+wsE
            V+lcelthF25ts78Tk+o4S5TcQwWCs1HHQAXjF+GyDjITMVjDHyJd2
            uEdPCDStoYVeA+niMy5VzSpCXhGpGE7HIfz8WUN72M8Lunw0UrME4
            H6NF7DsSKvV+BmXKBc6V2WCPfX8JSoVm8pN8nURMbAKBHArfaKMFv
            E28NikgJ3JEXbMC33wQAeFbX/DTH19OJ60RNuwTi8JJrPNHEl3+X0
            jMJicfJXtTjs3aLnN4vRnByB2wxXyKXYnH7vwDWG54UncgTmi2vah
            T/xoEwLyPaCPpyOl5PS8TmC89Avgm+BGL3uE11voRjBVie5LKaKMr
            8KZycCRRnXEk0PXC3cN4Sr8HY68SZcLObEA1ik9RW0RL0CkXdFp2y
            ImWGjGDr3pf3JousNJQKlkG/H7TBBjFgzcQO2JuP9Iqqni+o2pYLO
            SgR6RHds9vdmp5wosqCe1qrorCY8EqgSA/vF/4Id+A3PpvU1on7sS
            fuDVQiUqXwDIuc3ZNb2Gg4+Ij4mZ97ninQ8pwqZdgQaXTyl0rAMTs
            V1Senj+AmPiALUfBaLHvCQqO/LMelIESBcejDz3JPbX5jbX1RW8b+
            sK9Ozj8hTgQAAAABJRU5ErkJggg==
        """ )
