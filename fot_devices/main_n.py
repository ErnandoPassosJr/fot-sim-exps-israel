...
parser.add_argument('--dataset', action = 'store', dest = 'dataset', required = False)
parser.add_argument('--datasetColumn', action = 'store', dest = 'datasetColumn', required = False)
...
def init_sensor_agent(tatu_msg):
    global objGl
    objGl["topicAgent"]="iot/edge/raw/"+tatu_msg["sensor_type"]+"/"+tatu_msg['device_id']
    p = tatu_process(objGl,msgGl,'id'+str(args.port),None)
    #print("ID Processo", process_id)
    procs.append(p)
    p.start()

def stop_sensor_agent():
    for proc in procs:
        proc.terminate()
        continue
        procs.remove(proc)
